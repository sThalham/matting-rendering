...
cam_obj_bg_setting="${D}cl_x=0.0 ${D}cl_y=0.0 ${D}cl_z=-1.0 ${D}lk_x=0.0 ${D}lk_y=0.0 ${D}lk_z=0.00 ${D}cs_x=0.00 ${D}cam_a=1.777777777777 ${D}cam_z=-1.0 ${D}bg_sc=3.00 ${D}bg_pz=3.02 ${D}bg_rx=0.00 ${D}bg_ry=0.00 ${D}bg_rz=0.00 ${D}Dim=0.89 ${D}Trans=0.99 ${D}SC=0.85 ${D}IOR=1.48 ${D}RotZ=-31.61 ${D}RotY=-3.24 ${D}TX=0.00 ${D}TY=-1.10 ${D}FadeD=1.63 ${D}FadeP=1001.00"
echo $cam_obj_bg_setting

# Render images, object mask and attenuation mask
mkdir -p $outDir
sed -e  's#\${ImageName}#"custom_data/000000.jpg"#' ${template} > ${setting}
...
sed -e  's#\${ImageName}#"./custom_data/graycode_512_512/graycode_"#' ${template} > ${setting}
